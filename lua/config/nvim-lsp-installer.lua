...
-- 使用cmp_nvim_lsp 代替内置的omnifunc, 获取更强的补全体验
...
local function attach(_, bufnr)
    ...
    bmap(bufnr, "n", "gd", "<cmd>Telescope lsp_definitions theme=dropdown<CR>", opt)
    -- 列出光标下所有引用（代替内置 LSP 的窗口，telescope 插件让查看引用更方便）
    bmap(bufnr, "n", "gr", "<cmd>Telescope lsp_references theme=dropdown<CR>", opt)
    -- 工作区诊断（代替内置 LSP 的窗口，telescope 插件让工作区诊断更方便）
    bmap(bufnr, "n", "go", "<cmd>Telescope diagnostics theme=dropdown<CR>", opt)
    -- 显示代码可用操作（代替内置 LSP 的窗口，telescope 插件让代码行为更方便）
    bmap(bufnr, "n", "<leader>ca", "<cmd>Telescope lsp_code_actions theme=dropdown<CR>", opt)
    -- 变量重命名（代替内置 LSP 的窗口，Lspsaga 让变量重命名更美观）
    bmap(bufnr, "n", "<leader>cn", "<cmd>Lspsaga rename<CR>", opt)
    -- 查看帮助信息（代替内置 LSP 的窗口，Lspsaga 让查看帮助信息更美观）
    bmap(bufnr, "n", "gh", "<cmd>Lspsaga hover_doc<CR>", opt)
    -- 跳转到上一个问题（代替内置 LSP 的窗口，Lspsaga 让跳转问题更美观）
    bmap(bufnr, "n", "g[", "<cmd>Lspsaga diagnostic_jump_prev<CR>", opt)
    -- 跳转到下一个问题（代替内置 LSP 的窗口，Lspsaga 让跳转问题更美观）
    bmap(bufnr, "n", "g]", "<cmd>Lspsaga diagnostic_jump_next<CR>", opt)
